...
FROM world
WHERE population NOT BETWEEN (SELECT population FROM world WHERE name = 'Canada') AND (SELECT population FROM world WHERE name = 'Poland');
...
SELECT name, continent
FROM world x
WHERE population > ALL(SELECT MAX(y.population)*3  FROM world y WHERE y.continent = x.continent AND y.name <> x.name);
